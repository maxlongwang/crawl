...
from manager.items import FundManagerItem
...
class FundManagerSpider(scrapy.Spider):
    ...
    postdata = {"regiProvinceFsc":"province","offiProvinceFsc":"province","establishDate":{"from":"1900-01-01","to":"9999-01-01"},"registerDate":{"from":"1900-01-01","to":"9999-01-01"}}
    ...
    def gotourl_parse(self, response, **kwargs):
        ...
        manageritem['businessType'] = re.sub(' +',' ', sel.xpath('//td[contains(text(),"业务类型")]/following-sibling::td/text()').extract_first().strip().replace(u'\xa0\n',u''))
