...
async def upgrade(db: BaseDBAsyncClient) -> str:
    ...
    `name` VARCHAR(322) NOT NULL COMMENT 'classname'
) CHARACTER SET utf8mb4;
CREATE TABLE IF NOT EXISTS `student` (
    `id` INT NOT NULL PRIMARY KEY AUTO_INCREMENT,
    `name` VARCHAR(322) NOT NULL COMMENT 'username',
    `pwd` VARCHAR(322) NOT NULL COMMENT 'password',
    `sno` INT NOT NULL COMMENT 'xuhao',
    `clas_id` INT NOT NULL,
    CONSTRAINT `fk_student_clas_4be9b492` FOREIGN KEY (`clas_id`) REFERENCES `clas` (`id`) ON DELETE CASCADE
) CHARACTER SET utf8mb4;
CREATE TABLE IF NOT EXISTS `teacher` (
    `id` INT NOT NULL PRIMARY KEY AUTO_INCREMENT,
    `name` VARCHAR(322) NOT NULL COMMENT 'teachname',
    `pwd` VARCHAR(322) NOT NULL COMMENT 'password',
    `tno` INT NOT NULL COMMENT 'teach no'
) CHARACTER SET utf8mb4;
CREATE TABLE IF NOT EXISTS `course` (
    `id` INT NOT NULL PRIMARY KEY AUTO_INCREMENT,
    `name` VARCHAR(322) NOT NULL COMMENT 'coursename',
